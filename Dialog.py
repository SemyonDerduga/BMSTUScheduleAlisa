...
def handle_dialog(req, res):
    ...
    IBM = ['ибм','ибп','abm','iban','ibm','инженерный','бизнес','менеджмент','инженерныйбизнесименеджмент','бизнесменеджмент','инженерныйбизнесменеджмент']
    IU = ['иу','йоу','иву','иум','ио','иван','иу','информатика','информатикаисистемыуправления','информатикасистемыуправления','информационноеуправление','информационная','изюм']
    MT = ['мт','мета','машиностроительныетехнологии','технологиимашиностроения','машиностроение','мтс','мта','мрт']
    RK = ['рк','робототехникаикомплекснаяавтоматизация','робототехника','комплекснаяавтоматизация','автоматизация','робототехникакомплекснаяавтоматизация','вк','рпа','река']
    RL = ['рл','орел','орёл','рфпл','радиоэлектроника','радиоэлектроникаилазернаятехника','лазернаятехника','лазеры','радиоэлектроникалазерная']
    ...
    if state == 'base':
        ...
        if req['request']['original_utterance'] == 'Какие пары в понедельник?':
            ...
            if weekday_target >= weekday:
                date_today = (datetime.datetime.today() + datetime.timedelta(days=(weekday_target - weekday))).strftime('%Y-%m-%d')
            else:
                date_today = (datetime.datetime.today() + datetime.timedelta(days=((7-weekday+weekday_target)))).strftime('%Y-%m-%d')
            rasp = get_schedule_by_date(group_id, date_today)
            res['response']['text'] = rasp
            res['response']['buttons'] = base_buttons
            return
    ...
    if state == 'welcome':
        ...
        if req['request']['original_utterance'] == '':
            res['response']['text'] = 'Привет! Я буду помогать тебе вспоминать к какой паре вставать или когда ты освободишься, а ещё я знаю всё расписание бауманки! А теперь назови свой факультет.'
            res['response']['buttons'] = facultet_buttons
            return
        ...
        for facultet in facultets_sound:
            if facultet_name in facultet:
                state = 'get_facultet'
                res['response']['text'] = 'Ваш факультет ' + facultet[0].upper() + '? Если всё верно то назови номер кафедры. Если произошла ошибка скажите об этом.' 
    ...
    if state == 'get_facultet':
        ...
        try:
            cafedra_number = int(''.join(req['request']['nlu']['tokens']).rstrip(string.punctuation).lower().replace(' ', ''))
        except ValueError:
            res['response']['text'] = 'Нужно назвать только номер кафедры например для ИУ3 скажи 3. а ты сказал' + req['request']['original_utterance'].rstrip(string.punctuation).lower().replace(' ', '')
            res['response']['buttons'] = number_caf_buttons
            return
        ...
        res['response']['text'] = 'Твоя кафедра ' + facultet + cafedra_number + ' верно? Теперь нужно назвать номер группы!'
        return
    ...
    if state == 'get_cafedra':
        ...
        try:
            group_number = int(''.join(req['request']['nlu']['tokens']).rstrip(string.punctuation).lower().replace(' ', ''))
        except:
        ...
        res['response']['text'] = 'Твоя группа ' + facultet + cafedra_number + '-' + group_number + ' верно? Осталось сказать Бакалавр ты Аспирант или же Магистр! Выбирай с умом!'
    ...
    if state == 'get_group':
            ...
            if stepen_name in stepen:
                ...
                if str(group_id+".ics") not in files:
                    ...
                    if str(group_id[:-1]+"Л.ics") not in files:
                        res['response']['text'] = 'Извини, я не нахожу твою группу, давай попробуем ещё раз. Какой у тебя факультет?'
                        res['response']['buttons'] = facultet_buttons
                        db.delete(user_id)
                        state = 'welcome'
                        return
                else:
                    db.set(user_id, str(state+":"+facultet+":"+cafedra_number+":"+group_number+":"+str(stepen_name.upper()[0])))
                    if str(group_id+".ics") not in files:
                        res['response']['text'] = 'Извини, я не нахожу твою группу, давай попробуем ещё раз. Какой у тебя факультет?'
                        res['response']['buttons'] = facultet_buttons
                        db.delete(user_id)
        ...
        degree = str(db.get(user_id).decode()).split(':')[4]
        if degree == "Л":
            res['response']['text'] = 'Твоя группа ' + facultet + cafedra_number + '-' + group_number + ' верно? Теперь ты можешь спрашивать меня о расписании. Также ты можешь в любой момент сменить группу!'
            res['response']['buttons'] = base_buttons
            return
        res['response']['text'] = 'Твоя группа ' + facultet + cafedra_number + '-' + group_number + degree + ' верно? Теперь ты можешь спрашивать меня о расписании. Также ты можешь в любой момент сменить группу!'
        res['response']['buttons'] = base_buttons
        return
